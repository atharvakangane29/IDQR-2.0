  ...
  const startDate = new Date("2024-07-07"); // overridden at runtime by _dataEpoch
  ...
  function initWithData(json) {
    ...
    // If using uploaded CSV data, rebuild time series labels from real epoch
    if (window._dataEpoch) {
      TIME_LABELS.length = 0;
      BASELINE.length    = 0;
      REGIONAL_BASE.length = 0;

      // Determine span from actual data weekIdx range
      const idxs   = ALL_DATA.map(r => r.weekIdx).filter(n => !isNaN(n));
  ...
  function renderTable(data) {
    ...
    tbody.innerHTML = tableDisplayData.map(r => {
      ...
      return `<tr data-id="${r.id}"
        ...
        <td>${r.rowid !== undefined ? String(r.rowid).padStart(8, '0') : String(r.id).padStart(8, '0')}</td>
        <td>${r.name}</td>
        <td>${r.region}</td>
  ...
  function updateStateDropdown(baseData) {
    ...
    const states = Array.from(new Set(baseData.map(r => r.state || '').filter(Boolean))).sort();
  ...
  function applyFilters() {
    ...
    if (filters.state !== "All States") {
      data = data.filter(r => (r.state || r.region || "").includes(filters.state));
    }
  ...
  const OPENAI_KEY = "";
  const _chatHistory = [];
  let _searchMode = 'filter'; // 'filter' | 'ask'

  function setSearchMode(mode) {
    _searchMode = mode;
    const fBtn = document.getElementById('modeFilter');
    const aBtn = document.getElementById('modeAsk');
    if (!fBtn || !aBtn) return;
    if (mode === 'filter') {
      fBtn.style.background = '#2C3B4D'; fBtn.style.color = '#EEE9DF';
      aBtn.style.background = 'transparent'; aBtn.style.color = '#999';
      document.getElementById('nlSearch').placeholder = 'Filter records — e.g. show critical ones in IL…';
    } else {
      aBtn.style.background = '#FFB162'; aBtn.style.color = '#1B2632';
      fBtn.style.background = 'transparent'; fBtn.style.color = '#999';
      document.getElementById('nlSearch').placeholder = 'Ask anything — how many, why, compare, explain…';
    }
  }

  async function runNLSearch() {
    const q = document.getElementById("nlSearch").value.trim();
    ...
    // Build a compact data snapshot for the LLM (max 80 records to stay within token limits)
    const sample = currentData.slice(0, 80).map(r => ({
      id: r.id, name: r.name, region: r.region, state: r.state,
      type: r.type, score: r.score, reason: r.reason,
      accounttype: r.accounttype, vol: r.vol
    }));

    const modeInstruction = _searchMode === 'filter'
      ? `The user has selected FILTER mode. Always return mode: "filter" with matchedIds. If no records match, return empty matchedIds and explain in summary.`
      : `The user has selected ASK mode. Always return mode: "answer" with a plain-English summary. Never return matchedIds. Just answer the question directly and clearly.`;

    const systemPrompt = `You are an anomaly data analyst assistant embedded in a pharma sales dashboard.
  ${modeInstruction}

  Always respond with ONLY valid JSON in one of these two shapes:
    { "mode": "filter", "matchedIds": [array of numeric IDs], "summary": "brief explanation" }
    { "mode": "answer", "summary": "your answer in plain English" }

  No markdown, no text outside the JSON.
  Current dataset: ${currentData.length} records. Score range: ${filters.sevLo}–${filters.sevHi}.`;
  ...
  Records: ${JSON.stringify(sample)}`;
    ...
    try {
      ...
      if (summaryText) summaryText.textContent = parsed.summary || "";

      const ids = new Set((parsed.matchedIds || []).map(Number));
      if (ids.size > 0) {
        setCF(ids, "table");
        showToast(`AI found ${ids.size} record${ids.size !== 1 ? "s" : ""}`);
      } else {
        showToast("AI found no matching records");
      }
    } catch(err) {
      if (loader) loader.style.display = "none";
      if (summaryText) summaryText.textContent = "Search failed: " + err.message;
